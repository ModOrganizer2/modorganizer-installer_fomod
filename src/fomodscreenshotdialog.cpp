...
// Disables the native dotted selection border around focused elements. Unfortunately, disabling it via
// stylesheets is broken in the latest version of Qt
...
FomodScreenshotDialog::FomodScreenshotDialog(QWidget *parent, std::vector<std::pair<QString, QString>> carouselImages, int carouselIndex)
  : QDialog(parent, Qt::FramelessWindowHint)
  , ui(new Ui::FomodScreenshotDialog)
  , m_carouselImages(carouselImages)
{
  ...
  connect(carouselList, &QTableWidget::itemSelectionChanged, this, &FomodScreenshotDialog::selectedScreenshotChanged);
...
{
  int selectedColumn = getSelectedScreenshot();
  if (selectedColumn == m_carouselImages.size()-1) { return; }
...
void FomodScreenshotDialog::selectedScreenshotChanged()
{
  ...
  ui->slideshowPosition->setText(QString("%1/%2").arg(QString::number(selectedColumn + 1), QString::number(m_carouselImages.size())));
  ...
  for (int column = 0; column < ui->carouselList->columnCount(); column++) {
    ...
    if (column == selectedColumn) {
      scaleLabel->setStyleSheet(scaleLabel->styleSheet() + "QLabel { border:2px solid white; }");
    }
    else {
